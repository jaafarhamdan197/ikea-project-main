...
# Load data
try:
    df = pd.read_excel('IKEA_RAW_DATA.xlsx')
    
    # Normalize platform names (fix case inconsistency)
    df['platform'] = df['platform'].str.upper()
    
    data_loaded = True
    print(f"Data loaded successfully. Found {len(df)} rows and {len(df['platform'].unique())} unique platforms.")
...
def optimize_budget(total_budget, curve_params, funnel_weights):
    ...
    if not curve_params or total_budget <= 0:
        print("No curve parameters or budget <= 0")
        return pd.DataFrame()
    ...
    for key in curve_params.keys():
        if '_' in key:
            parts = key.split('_')
            # The last part is the funnel, everything before is the platform
            funnel = parts[-1]
            platform = '_'.join(parts[:-1])
            platform_funnels.append((platform, funnel))
    
    print(f"Platform-funnel combinations for optimization: {platform_funnels}")
    
    if not platform_funnels:
        print("No platform-funnel combinations found")
        return pd.DataFrame()
    ...
    for i, (platform, funnel) in enumerate(platform_funnels):
        ...
        if pf in curve_params:
            ...
            # Convert platform key back to display name
            display_platform = platform.replace('_', ' ')
    ...
    print(f"Optimization results: {len(results_df)} platform-funnel combinations")
    return results_df
...
# Load curve parameters based on the data
if data_loaded:
    # Get unique platforms and funnel levels from the data
    platforms = df['platform'].unique()
    funnel_levels = df['communication_type'].unique()
    
    print(f"Building curve parameters for {len(platforms)} platforms and {len(funnel_levels)} funnel levels")
        ...
        for funnel in funnel_levels:
            ...
            if mask.any():
                ...
                if avg_spend > 0 and avg_revenue > 0:
                    ...
                    # Store parameters
                    # Use a simplified platform name for the key
                    platform_key = platform.replace(' ', '_')
                    curve_params[f"{platform_key}_{funnel}"] = {
                        'V_max': V_max,
                        'k': k,
                        'n': n
                    }
                    print(f"Created curve parameters for {platform_key}_{funnel}")
    
    curves_loaded = True
    print(f"Created curve parameters for {len(curve_params)} platform-funnel combinations")
    print(f"Curve parameter keys: {list(curve_params.keys())}")
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            ...
            html.Div(id='budget-display', style={'marginTop': '5px', 'marginBottom': '20px', 'color': 'white'}),
            ...
            # Constant forecast period note
            html.Div("Forecast Period: 30 days", style={'color': 'white', 'marginBottom': '20px'}),
        ...
        html.Div([
            ...
            dcc.Tabs([
                dcc.Tab(label='Revenue Forecast', children=[
                    html.Div([
                        html.Div(id='total-revenue-display', style={'fontSize': '24px', 'marginBottom': '20px', 'padding': '15px', 'backgroundColor': '#333', 'borderRadius': '5px', 'color': 'white'}),
                ...
                dcc.Tab(label='Debug Info', children=[
                    html.Div([
                        html.H4("Data and Model Information", style={'marginBottom': '15px', 'color': 'white'}),
                        html.Pre(id='debug-info', style={'whiteSpace': 'pre-wrap', 'backgroundColor': '#333', 'color': '#00FF00', 'padding': '15px', 'borderRadius': '5px', 'maxHeight': '500px', 'overflow': 'auto'})
                    ], style={'padding': '20px'})
                ]),
            ]),
    ...
    html.Div([
        ...
        html.P(f"Number of Platform-Funnel Combinations: {len(curve_params)}", style={'color': 'white'}),
        html.P("Dashboard created based on IKEA RAW DATA and model instructions.", style={'color': 'white'}),
...
def update_predictions(n_clicks, budget, funnel_level):
    # Set constant forecast days
    days = 30
...
Curves Loaded: {curves_loaded}
Number of Curve Parameters: {len(curve_params)}
Curve Parameter Keys: {list(curve_params.keys())}
Selected Funnel: {funnel_level}
...
- Number of Results: {len(optimal_allocation)}
- Platforms in Results: {optimal_allocation['Platform'].unique() if not optimal_allocation.empty else 'None'}
- Funnels in Results: {optimal_allocation['Funnel'].unique() if not optimal_allocation.empty else 'None'}
    """
    ...
    if len(optimal_allocation) > 0:
        ...
        debug_info += f"After Funnel Filter: {len(optimal_allocation)} results\n"
    ...
    for spend in spend_grid:
        ...
        if len(optimal_allocation) > 0:
            ...
            for i, row in optimal_allocation.iterrows():
                ...
                funnel = row['Funnel']
                # Convert platform name to key format
                platform_key = platform.replace(' ', '_')
                pf = f"{platform_key}_{funnel}"
                
                if pf in curve_params:
                    params = curve_params[pf]
                    platform_spend = spend * proportions.iloc[i]
    ...
    return (metrics_cards, dim_returns_fig, allocation_fig, platform_fig, 
            total_revenue_text, dim_returns_insight, allocation_table, platform_insights_text, debug_info)
